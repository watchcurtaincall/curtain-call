...
export default function SignupPage() {
  ...
  // Simulated Google Sign-In state
  const [showGoogleModal, setShowGoogleModal] = useState(false);
  const [googleEmail, setGoogleEmail] = useState('theatre.lover@gmail.com');
  const [googleName, setGoogleName] = useState('Theatre Lover');
  ...
  const handleGoogleSubmit = async (e: React.FormEvent) => {
    ...
    await new Promise(r => setTimeout(r, 1200));
    login(googleEmail, undefined, googleName);
    ...
    <div className="container mx-auto px-4 py-12 md:py-24 flex items-center justify-center min-h-[calc(100vh-16rem)]">
      <div className="w-full max-w-md bg-zinc-900 border border-white/10 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <button
            ...
            className="mt-4 w-full bg-zinc-800 border border-white/5 text-white font-medium py-3 rounded-xl hover:bg-zinc-700 transition-colors flex items-center justify-center gap-2"
          >
      ...
      {showGoogleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-md" onClick={() => setShowGoogleModal(false)} />
          <div className="relative w-full max-w-md bg-zinc-950 border border-white/10 rounded-3xl p-8 shadow-2xl relative overflow-hidden animate-scale-up">
            ...
            <div className="relative z-10">
              <div className="text-center mb-6">
                ...
                <h2 className="font-serif font-bold text-white text-xl">Sign in with Google</h2>
                <p className="text-sm text-zinc-400 mt-1">Simulated Google OAuth authentication provider.</p>
              </div>
              ...
              <form onSubmit={handleGoogleSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    type="email"
                    required
                    placeholder="e.g. theatre.lover@gmail.com"
                    value={googleEmail}
                    onChange={e => setGoogleEmail(e.target.value)}
                    className="w-full bg-zinc-900 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:border-red-500 transition-all"
                  />
                </div>
                ...
                <div className="flex gap-3 pt-4">
                  <button
                    ...
                    className="flex-1 bg-zinc-900 border border-white/10 text-white font-medium py-3 rounded-xl hover:bg-zinc-800 transition-colors text-sm"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="flex-1 bg-white text-black font-bold py-3 rounded-xl hover:bg-zinc-100 transition-colors text-sm flex items-center justify-center gap-1.5"
                  >
                    {googleLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Continue'}
                  </button>
                </div>
              </form>
            </div>
